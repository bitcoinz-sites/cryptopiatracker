...
function updatePage(){
    ...
    var DotBalance1_toBtc = (DotBalance1*DotValue);
    document.getElementById('dotBalance1').innerHTML = DotBalance1.toFixed(0) + " DOT";
    document.getElementById('dotBalance1_toBtc').innerHTML = "<b>" + DotBalance1_toBtc.toFixed(6) + " BTC</b>";	
	...
	var TotalBalance2 = BtczBalance2_toBtc + BtcBalance2 + LtcBalance2_toBtc + EthBalance2_toBtc + ZecBalance2_toBtc + DotBalance1_toBtc;	
	document.getElementById('totalBalance2').innerHTML = "<b>" + TotalBalance2.toFixed(6) + " BTC</b>";
	
	document.getElementById('progressBar2').style.width = (TotalBalance2/0.05).toFixed(2) + "%";
    document.getElementById('progressPercent2').innerHTML = (TotalBalance2/0.05).toFixed(1) + "%";
...
function getBalances(){
    ...
    xhr15.open("GET", "https://api.coinmarketcap.com/v1/ticker/dotcoin/", true);
    xhr15.onload = function(){
        if(this.status == 200){
            var result = JSON.parse(xhr15.responseText);
            DotValue = Number(result[0].price_btc);
            updatePage();
        }
    }	
    xhr15.send();
